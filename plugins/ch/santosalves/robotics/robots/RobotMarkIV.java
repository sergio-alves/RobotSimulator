package ch.santosalves.robotics.robots;

import ch.santosalves.robotics.inputs.RangeDetectors;
...
public class RobotMarkIV extends GenericRobot{
    ...
    //private boolean haveBeenScanned = false;
    ...
    private void appendInputs() {
        getInputs().add(new ch.santosalves.robotics.inputs.VoidDetector());
        getInputs().add(new ch.santosalves.robotics.inputs.ObstacleDetector());
    }//end appendInputs
    ...
    @Override
    public void start() {
        //Instanciate a new map that corresponds to the playground and set the
        //robot position
    //    setMap(new RobotWorldMap(playground.getPlaygroundWidth(), playground.getPlaygroundHeight()));
     ...
     //       input.setPlayground(playground);
       // });
        ...
        //Finally start exploration and simultanously the execution of enqueued commands        
        //startExploring(new TreeLikeExploringAlgorithm(this));
        //startExecuting(new ExecutingAlgorithm(this));
        ...
        start(new Runnable() {
            ...
            private void algorithmF() {
                GenericRobot.log.info("Next case is an obstacle|visite|start_point.");
            ...
            public void run() {
                ...
                while (true) {
                    ...
                    GenericRobot.log.finest("MAP VALUE for (x,y)=>(" + next.getX() + ", " + next.getY() + ") : " + cst);
                    switch (cst) {
